...
Notice that the callback function for `insertOne` has a `result` argument. This is an object that has some useful information about what you inserted into the collection. One useful property is `result.ops`, which has an array with all the documents you just created, including the automatic `_id`.
...
There are several options you can use. The one that we will use in this lesson is `returnOriginal`. If your options include `returnOriginal: false`, then you can get the updated document from `result`.
...
On this page, on the left side bar, we can see many objects we can use with the `mongodb` package. We want to find information about the `findOneAndUpdate` function, which is on a `Collection`, so let's click the "Collection" link. This opens many properties that a `Collection` has, including its methods (functions). Find and click the link for `findOneAndUpdate`.
...
Note: at the time of writing this lesson, the docs have some problems. For example, the description of the `value` property says that it is from the `findAndModify` command, but it is really from the `findAndUpdateOne` command for us. This kind of thing is not uncommon in documentation. A good place to ask for help if documentation does not work is [Stack Overflow](https://stackoverflow.com).
...
To try it, find a film title in your database, and send a `DELETE` request to `/films/<put the film title here>` in Postman.
