...
insert a document in sample_airbnb.listingsAndReviews with the following fields:
...
* You can use the code that we developed in lesson 2, or the GUI (hint: clone)


## Indexing

<steps to do so in GUI>
...
<steps to do so in GUI, add built in role>

<steps to do so in GUI, add custom role, read only in airbnb db>
